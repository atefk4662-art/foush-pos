...
function renderPOS(c) {
    ...
    // If no active shift, show opening overlay
    if (!shift && window.state.role !== 'waiter') {
        c.innerHTML = `
            <div class="view-container" style="display:flex; align-items:center; justify-content:center; height:calc(100vh - 4.5rem);">
                <div class="glass-panel" style="text-align:center; padding:4rem; max-width:500px; border:2px solid var(--primary); box-shadow: 0 0 50px rgba(245, 158, 11, 0.15);">
                    <div style="font-size:5rem; margin-bottom:2rem; filter: drop-shadow(0 0 15px var(--primary));">🔑</div>
                    <h2 style="font-size:2rem; font-weight:900; margin-bottom:1rem; color:var(--primary);">الوردية مغلقة</h2>
                    <p style="color:var(--text-dim); margin-bottom:2.5rem; line-height:1.6;">يجب فتح وردية جديدة لبدء العمل وتسجيل الطلبات والمصروفات.</p>
                    <button onclick="window.startShift()" class="btn-luxury" style="width:100%; padding:1.5rem; font-size:1.3rem;">
                        <i class="fa-solid fa-play"></i> فتح وردية الآن
                    </button>
                </div>
            </div>
        `;
        ...
        <div class="view-container" style="padding:1.5rem; overflow-y:auto; height:calc(100vh - 4.5rem);">
            ...
            <div style="display:grid; grid-template-columns: 1fr 2fr; gap:1.5rem; margin-bottom:2rem;">
                ...
                <div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:1rem;">
                    <div class="glass-panel" style="text-align:center; display:flex; flex-direction:column; justify-content:center; background:rgba(239, 68, 68, 0.08); border-bottom:4px solid var(--danger);">
                        <div style="font-size:0.8rem; opacity:0.6; margin-bottom:8px;">إجمالي المصروفات</div>
                        <div style="font-size:1.8rem; font-weight:900; color:var(--danger);">${expensesTotal} <span style="font-size:0.7rem;">ج.م</span></div>
                    </div>
                    <div class="glass-panel" style="text-align:center; display:flex; flex-direction:column; justify-content:center; background:rgba(99, 102, 241, 0.08); border-bottom:4px solid var(--accent);">
                        <div style="font-size:0.8rem; opacity:0.6; margin-bottom:8px;">طلبات اليوم</div>
                        <div style="font-size:1.8rem; font-weight:900; color:var(--accent);">${shiftOrders.length}</div>
                    </div>
                    <div class="glass-panel" style="text-align:center; display:flex; flex-direction:column; justify-content:center; background:rgba(245, 158, 11, 0.08); border-bottom:4px solid var(--primary);">
                        <div style="font-size:0.8rem; opacity:0.6; margin-bottom:8px;">نشط حالياً</div>
                        <div style="font-size:1.8rem; font-weight:900; color:var(--primary);">${activeOrders.length}</div>
                    </div>
                    <div class="glass-panel" style="text-align:center; display:flex; flex-direction:column; justify-content:center; background:rgba(16, 185, 129, 0.08); border-bottom:4px solid var(--success);">
                        <div style="font-size:0.8rem; opacity:0.6; margin-bottom:8px;">سرعة التحضير</div>
                        <div style="font-size:1.8rem; font-weight:900; color:var(--success);">${avgPrepTime} <span style="font-size:0.7rem;">دقيقة</span></div>
                    </div>
                </div>
            </div>
    ...
    c.innerHTML = `
        <div class="view-container" style="padding:1.5rem; overflow-y:auto; height:calc(100vh - 4.5rem);">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:2rem;">
                <h1 style="font-weight:900; margin:0; color:var(--primary);">إدارة المخزون 📦</h1>
                <div style="opacity:0.6; font-weight:bold;">إجمالي الأصناف: ${menu.length}</div>
            </div>
...
window.renderSidebar = () => {
    try {
        ...
        const links = {
            ...
            cashier: [{id:'pos',label:'نقطة البيع',icon:'cash-register'},{id:'inventory',label:'المخزون',icon:'box'},{id:'dashboard',label:'المتابعة',icon:'chart-line'},{id:'expenses',label:'مصروفات',icon:'money-bill-transfer'}],
